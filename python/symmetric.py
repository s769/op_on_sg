...
# This file contains functions that generate coefficients for the symmetric orthogonal polynomials with respect to 
...
def d_pnj1(nn, j, i):
  '''
    This function calculates the value of the normal derivative of P_{j,1}^(n) at q_i
    Args:
      nn: corresponds to the value of (n) in P_{j,1}^(n)
      j: corresponds to the value of j in P_{j,1}^(n)
      i: corresponds to the boundary index q_i
...
def inner_pnj1pmk1(nn, j, m, k):
  '''
    This function calculates the value of the L2 inner product <P_{j,1}^(n), P_{m, 1}^(k)>
    Args:
      nn: corresponds to the value of (n) in P_{j,1}^(n)
      j: corresponds to the value of j in P_{j,1}^(n)
...
def inner_rjrk(j, k, lam = np.array([1])):
  '''
        Computes any Sobolev inner product of <R_j, R_k> (the basis functions for the symmetric polynomials).
...
def generate_symm_ops(n, normalized=False, lam = np.array([1]), frac=True):
  '''
    Generates symmetric orthogonal polynomials with respect to a generalized Sobolev 
    ...
    Returns:
        np.array of coefficients of the orthogonal polynomials with 
            ...
            columns.
            If normalized is True, the polynomials will be normalized if . 
